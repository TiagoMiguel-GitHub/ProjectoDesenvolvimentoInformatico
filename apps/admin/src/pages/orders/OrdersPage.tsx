        ...
        {orders.map((o) => {
          ...
          return (
            <div key={o.id} style={s.trow}>
              ...
              <span>{o.fulfillment_type === "delivery" ? "🚚 Entrega" : "🏪 Levant."}</span>
              <span style={{ fontSize: 13 }}>{o.payment_method}</span>
              <span style={{ fontWeight: 700, color: "#2d6a4f" }}>€{Number(o.total).toFixed(2)}</span>
              <span style={{ background: c + "22", color: c, padding: "3px 10px", borderRadius: 12, fontSize: 12, fontWeight: 600 }}>{STATUS_LABELS[o.status]}</span>
              <span style={{ fontSize: 13, color: "#888" }}>{new Date(o.created_at).toLocaleDateString("pt-PT")}</span>
...
const s: Record<string, React.CSSProperties> = {
  ...
  viewBtn: { background: "#2d6a4f", color: "#fff", border: "none", padding: "6px 12px", borderRadius: 6, cursor: "pointer", fontSize: 13 },
  overlay: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 },
  modal: { background: "#fff", borderRadius: 16, padding: 32, width: 480, display: "flex", flexDirection: "column", gap: 8, maxHeight: "90vh", overflowY: "auto" },
